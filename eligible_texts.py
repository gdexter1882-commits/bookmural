            ...
            for row in reader:
                try:
                    ...
                    if layout.get("eligible"):
                        # Extract suffix for CDN filename
                        match = re.search(r"(crane_stephen|dickens_charles|forster_em|hardy_thomas|johnson_samuel|joyce_james|mansfield_katherine|wells_hg|wilde_oscar|wodehouse_pg|woolf_virginia)_.+$", handle)
                        cover_key = match.group(0) if match else handle
