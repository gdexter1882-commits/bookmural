...
# This ensures that if the JSON key requires "  (127)", the CSV title is created as 
# "  (127 pages)" by the synchronizer, and this regex removes only the " (127 pages)" part,
# leaving the rest of the folder path intact, including the necessary trailing spaces.
    ...
    for margin_test in range(5, 16):
        ...
        for scale_pct in range(95, 106):
            ...
            for cols in range(1, max_cols + 1):
                ...
                # The total height includes the pages plus the gaps between rows.
            ...
            for row in reader:
                try:
                    ...
                    if layout['eligible']:
                        ...
                        # 2. GENERATE THE FOLDER KEY: Use regex to remove the (NUM pages) suffix.
                        # CRITICAL FIX: Removed .strip() to preserve necessary trailing whitespace 
                        # that the CSV synchronizer already inserted to match the CDN map key exactly.
                        folder_base = CSV_TITLE_COUNT_REGEX.sub('', title) # <--- .strip() REMOVED
                        
                        # Final CDN folder key must be in the format: 'BASE FOLDER (NUM)'
